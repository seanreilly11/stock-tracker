...
const AINotesList = ({ ticker, name, type, savedStock }: Props) => {
    ...
    const notesPromise = useMemo(
        () => (!isDev && ticker && type)
            ? fetchAINotes(ticker, type).catch(() => null)
            : Promise.resolve(null),
        [ticker, type, isDev],
    );
    const AINotes: AINotes[] | null = use(notesPromise);
    ...
    if (isDev) return <div className="space-y-1 mb-4"><p>No AI notes returned in dev</p></div>;
    if (!AINotes) return <p className="text-sm text-gray-400 mb-4">AI suggestions unavailable.</p>;
