...
export default function AuthModal({ open, mode, onClose, onSwitchMode }: AuthModalProps) {
  ...
  const canSubmit = validEmail && validPass && validName && validAgree && !submitting;

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!canSubmit) {
      if (!validEmail) setError("That email doesn’t look right.");
      else if (!validPass) setError("Password must be at least 8 characters.");
    ...
    try {
      ...
      const msg = err instanceof AuthError ? err.message : "Something went wrong";
      setError(
        msg.includes("Invalid login credentials") ? "Invalid email or password" :
    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: "0 1px 0 var(--rule-soft), 0 30px 80px -30px oklch(20% 0.02 60 / 0.35), 0 60px 160px -50px oklch(20% 0.02 60 / 0.2)",
      ...
      >
        ...
        <aside
          ...
          style={{
            background: "radial-gradient(80% 60% at 0% 100%, color-mix(in oklch, var(--accent-soft) 60%, transparent) 0%, transparent 70%), var(--paper-2)",
          }}
          aria-hidden="true"
        >
          ...
          <blockquote className="m-0 p-0 border-l-2 border-[var(--ink)] pl-4">
            <p className="m-0 font-[family-name:var(--serif)] text-[19px] leading-[1.45] tracking-[-0.005em] text-[var(--ink)] italic" style={{ textWrap: "pretty" } as React.CSSProperties}>
              &ldquo;The move is coming either way. The only question is whether it happens{" "}
              <em className="text-[var(--accent)]">to</em> you, or whether you meet it with a plan.&rdquo;
            </p>
          </blockquote>
          ...
          <ul className="list-none m-0 p-0 flex flex-col gap-3.5">
            {[
              ...
              <li key={item} className="flex items-start gap-3 font-[family-name:var(--serif)] text-[14.5px] text-[var(--ink-2)] leading-[1.45]">
        ...
        <section className="p-[30px_40px_32px] overflow-y-auto flex flex-col">
            ...
            {(["login", "register"] as Mode[]).map((m) => (
              <button
                ...
                style={{
                  ...
                  boxShadow: mode === m ? "0 1px 0 var(--rule-soft), 0 2px 6px oklch(20% 0.02 60 / 0.06)" : "none",
                }}
          ...
          <header className="mb-[22px]">
            ...
            <h2 className="m-0 mb-2 font-[family-name:var(--serif)] font-medium text-[34px] leading-[1.08] tracking-[-0.02em] text-[var(--ink)]">
              {isRegister ? "Start your notebook." : <>Pick up where<br />you left off.</>}
            </h2>
          ...
          <form className="flex flex-col gap-3.5" onSubmit={handleSubmit} noValidate>
              ...
              <div className="flex items-baseline justify-between">
                ...
                {!isRegister && (
                  <button
                    ...
                    onClick={() => setError("Check your email for a reset link.")}
                  >
                    Forgot?
                  </button>
                )}
            ...
            <div className="mt-0.5">
              {isRegister ? (
                <CheckRow checked={agree} onChange={setAgree}>
                  I agree to the{" "}
                  <a href="/terms" className="text-[var(--ink)] underline underline-offset-[2px] decoration-[var(--ink-4)] hover:decoration-[var(--accent)]">terms</a>{" "}
                  and the{" "}
                  <a href="/privacy" className="text-[var(--ink)] underline underline-offset-[2px] decoration-[var(--ink-4)] hover:decoration-[var(--accent)]">privacy promise</a>.
                </CheckRow>
              ) : (
            ...
            >
              {submitting
                ? (isRegister ? "Creating your notebook…" : "Signing in…")
            ...
            <div className="mt-3.5 font-[family-name:var(--serif)] text-[13.5px] text-[var(--ink-3)] text-center">
              {isRegister ? (
                <>Already have a notebook?{" "}
                  <button type="button" className="border-0 bg-transparent p-0 font-inherit text-[var(--ink)] cursor-pointer underline underline-offset-[3px] decoration-[var(--accent)] hover:text-[var(--accent)]" onClick={() => onSwitchMode("login")}>Sign in</button>
                </>
              ) : (
                <>New here?{" "}
                  <button type="button" className="border-0 bg-transparent p-0 font-inherit text-[var(--ink)] cursor-pointer underline underline-offset-[3px] decoration-[var(--accent)] hover:text-[var(--accent)]" onClick={() => onSwitchMode("register")}>Create an account</button>
                </>
              )}
            </div>
          </form>
...
function PasswordMeter({ value }: { value: string }) {
  ...
  const colors = ["", "var(--accent)", "oklch(70% 0.13 75)", "oklch(60% 0.12 130)", "var(--green)"];
  return (
  ...
  return (
    <label className="inline-flex items-start gap-2.5 font-[family-name:var(--serif)] text-[13.5px] text-[var(--ink-2)] leading-[1.4] cursor-pointer select-none relative">
      ...
      <span
        ...
        style={checked ? { background: "var(--ink)", borderColor: "var(--ink)" } : {}}
      >
        {checked && (
          <span
            className="absolute"
