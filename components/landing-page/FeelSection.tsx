...
const FeelSection = () => (
  <section className="py-24">
    <div className="max-w-[1200px] mx-auto px-10">
      ...
      <div className="grid gap-6 mt-8" style={{ gridTemplateColumns: "repeat(3, minmax(0,1fr))" }}>
        {/* Prepared */}
        <div className="bg-[var(--paper)] border border-[var(--rule)] p-[28px_26px_26px] flex flex-col gap-3.5">
          ...
          <p className="font-[family-name:var(--serif)] text-[15.5px] leading-[1.55] text-[var(--ink-2)] m-0 flex-1">
            A target isn&apos;t a number. It&apos;s a price you decided was worth acting on, plus
            the reasoning that got you there. When the price arrives, the reasoning arrives with
            it — so you act on judgment, not impulse.
          </p>
          {/* Target rail vis */}
              ...
              {[
                ...
                <span
                  ...
                  style={{
                    ...
                    background: p.kind === "now" ? "var(--ink)" : "var(--paper)",
                ...
                >
                  <span
                    ...
                    style={{ top: p.kind === "now" ? -16 : 14, color: p.kind === "now" ? "var(--ink)" : undefined, fontWeight: p.kind === "now" ? 500 : undefined }}
                  >
                    {p.label}
                  </span>
                </span>
        ...
        <div className="bg-[var(--paper)] border border-[var(--rule)] p-[28px_26px_26px] flex flex-col gap-3.5">
          ...
          <p className="font-[family-name:var(--serif)] text-[15.5px] leading-[1.55] text-[var(--ink-2)] m-0 flex-1">
            You&apos;re not watching prices all day. You&apos;re not refreshing the news. You wrote
            down what you&apos;re waiting for, and the notebook is watching. Open it when there&apos;s
          ...
          <div className="relative mt-3 p-[28px_16px_16px] bg-[var(--paper-2)] border border-[var(--rule-soft)] rounded min-h-[110px]">
            <div className="flex flex-col gap-2 mt-1">
              {[
                { kind: "plan",  when: "Jan 15", body: "Initial thesis. Buy on $108 pullback." },
                { kind: "risk",  when: "Feb 22", body: "Export rules — watch April ruling." },
                { kind: "alert", when: "Apr 22", body: "BUY $108 hit. Email sent." },
              ].map((item) => (
                <div key={item.when} className="grid items-center gap-2 text-[12px] min-w-0" style={{ gridTemplateColumns: "42px 46px 1fr" }}>
                  <span
                    ...
                    style={item.kind === "alert" ? { color: "var(--accent)", borderColor: "var(--accent-line)", background: "var(--accent-soft)" } : {}}
                  >
                    {item.kind}
                  </span>
                  <span className="font-[family-name:var(--mono)] text-[10px] text-[var(--ink-4)]">{item.when}</span>
                  <span className="font-[family-name:var(--serif)] text-[12.5px] text-[var(--ink-2)] whitespace-nowrap overflow-hidden text-ellipsis min-w-0">{item.body}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="bg-[var(--paper)] border border-[var(--rule)] p-[28px_26px_26px] flex flex-col gap-3.5">
          ...
          <p className="font-[family-name:var(--serif)] text-[15.5px] leading-[1.55] text-[var(--ink-2)] m-0 flex-1">
            A note from March says &quot;I&apos;m worried about export rules.&quot; A note from May
            says &quot;I forgot why I was worried.&quot; The notebook keeps both. Theses age —
            and watching yours age is how you catch a plan that&apos;s quietly stopped making sense.
          </p>
          {/* Email vis */}
          <div className="relative mt-3 p-[28px_16px_16px] bg-[var(--paper-2)] border border-[var(--rule-soft)] rounded min-h-[110px]">
            <div className="bg-[var(--paper)] border border-[var(--rule)] rounded p-[10px_12px]">
              ...
              <div className="font-[family-name:var(--mono)] text-[9.5px] text-[var(--ink-4)] mt-1.5">thesis · 98 days ago</div>
            </div>
          </div>
        </div>
      </div>
