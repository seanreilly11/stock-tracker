...
const StockListSection = async ({ searchParams }: StockListSectionProps) => {
  ...
  const counts = {
    all: stocks.length,
    alerts: stocks.filter((s) => (targetCounts.triggered[s.id] ?? 0) > 0)
      .length,
  ...
  if (filter === "alerts")
    visible = visible.filter((s) => (targetCounts.triggered[s.id] ?? 0) > 0);
  ...
  return (
    <section className="mt-10">
      ...
      {stocks.length === 0 ? (
        ...
        <p className="py-12 text-center text-sm text-[var(--ink-3)]">
          No stocks match.
        </p>
      ) : (
        <div className="mt-8">
