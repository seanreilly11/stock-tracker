              ...
              if (!stock) {
                return (
                  <li key={`empty-${i}`}>
                    ...
                    >
                      <span className="font-[family-name:var(--serif)] italic text-sm text-[var(--ink-4)] text-center">{i + 1}</span>
                      <span className="text-[12.5px] flex items-center gap-1.5">+ Pick a stock</span>
                    </button>
                  </li>
                );
              }
              ...
              return (
                <li key={stock.ticker} className="group">
                  <div className="grid items-center gap-2 sm:gap-3 px-1 py-2.5 border-b border-[var(--rule-soft)] hover:bg-[var(--paper-2)] transition-colors grid-cols-[16px_48px_1fr_auto] sm:grid-cols-[18px_52px_1fr_auto_22px]">
                    <span className="font-[family-name:var(--serif)] italic text-sm text-[var(--ink-4)] text-center">{i + 1}</span>
                    <Link href={`/stocks/${stock.ticker}`} className="font-[family-name:var(--mono)] font-medium text-[12.5px] text-[var(--ink)] tracking-[0.02em] hover:text-[var(--ink-2)]">{stock.ticker}</Link>
                    {stock.topBuyTarget ? (
                      <span className="font-[family-name:var(--mono)] text-[12px] text-[var(--ink-2)]">buy <strong className="text-[var(--ink)] font-medium">${stock.topBuyTarget.toFixed(2)}</strong></span>
                    ) : (
                      <span className="font-[family-name:var(--mono)] text-[12px] text-[var(--ink-3)]">no buy target{stock.mostRecentPrice ? ` · $${stock.mostRecentPrice.toFixed(2)}` : ""}</span>
                    )}
                    {distance !== null ? (
                      <span className={`font-[family-name:var(--mono)] text-[10.5px] uppercase tracking-[0.04em] text-right ${distance <= 0 ? "text-[var(--green)]" : "text-[var(--ink-3)]"}`}>
                        {distance <= 0 ? `${Math.abs(distance).toFixed(1)}% in range` : `${distance.toFixed(1)}% to go`}
                      </span>
                    ) : <span />}
                    <button
            ...
            {slots.map((stock, i) => {
                ...
                return (
                  <li key={`empty-${i}`} className="flex-1">
                    <button
                      ...
                      onClick={() => setShowModal(true)}
                    >
                      <span className="font-[family-name:var(--serif)] italic text-2xl text-[var(--ink-4)] leading-none">{i + 1}</span>
              ...
              return (
                <li key={stock.ticker} className="flex-1 group">
                  ...
                  >
                    <span className="font-[family-name:var(--serif)] italic text-2xl text-[var(--ink-4)] leading-none pt-0.5">{i + 1}</span>
                    <div className="min-w-0 flex-1">
                      <span className="block font-[family-name:var(--mono)] text-sm font-medium text-[var(--ink)]">{stock.ticker}</span>
                      {stock.topBuyTarget ? (
                        <span className="block font-[family-name:var(--mono)] text-[11px] text-[var(--ink-2)] mt-0.5">
                          buy <strong className="font-medium">${stock.topBuyTarget.toFixed(2)}</strong>
                        </span>
                      ) : (
                        <span className="block font-[family-name:var(--mono)] text-[11px] text-[var(--ink-3)] mt-0.5">no buy target</span>
                      )}
                      {distance !== null && (
                        <span className={`block font-[family-name:var(--mono)] text-[10px] uppercase tracking-wider mt-1 ${distance <= 0 ? "text-[var(--green)]" : "text-[var(--ink-4)]"}`}>
                          {distance <= 0 ? `${Math.abs(distance).toFixed(1)}% in range` : `${distance.toFixed(1)}% to go`}
                        </span>
                      )}
                    </div>
