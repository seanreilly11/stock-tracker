export const searchStocks = async (keyword: string) => {
  ...
  // );
  const res = await fetch(
    `https://api.polygon.io/v3/reference/tickers?search=${keyword}&market=stocks&active=true&sort=type&order=asc&limit=25&apiKey=${process.env.NEXT_PUBLIC_POLYGON_API_KEY}`,
  );
  return res.json();
};
