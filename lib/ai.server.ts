...
const parseAIJSON = (content: string): AINotes[] => {
    const cleaned = content.startsWith("```json")
        ? (content.split("```json")[1]?.split("```")[0] ?? content)
        : content;
    return JSON.parse(cleaned);
};

export const fetchAINotesServer = (
    ticker: string,
    type: string,
): Promise<AINotes[]> => {
    ...
    return getOpenAI()
        ...
        .then((response) => parseAIJSON(response.choices[0].message.content ?? ""));
};
