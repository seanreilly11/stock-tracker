...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    ...
    keywords: [ticker, name, sector, "stock tracker", "investment"].filter(
      Boolean,
    ),
    alternates: {
    ...
    openGraph: {
      ...
      description:
        description?.slice(0, 200) ?? `Track ${ticker} on ${APP_TITLE}`,
...
4. Metadata: Inspect `<head>` on `/stocks/AAPL` - title, description, og:_, twitter:_ all present
